...
def handle_attack_command(com, languages, attack_type,mode,more=''):
    ...
    if b != False:
        ...
        if c == 0:
           ...
           if more == 'COOKIE':a = a.replace('المتصفح','كوكيز').replace('نوع الوكيل','اختيار').replace('فقط SOCKS4، SOCKS5، HTTP، HTTPS, NONE','صفر للافتراضي، واحد لصفحة UAM').replace('tipe-proxy','opt').replace('HANYA SOCKS4, SOCKS5, HTTP, HTTPS, NONE','it 0 untuk default, 1 untuk halaman UAM').replace('ประเภทของproxy','เพิ่มเติม').replace('เบราว์เซอร์','คุกกี้').replace('แค่ SOCKS4, SOCKS5, HTTP, HTTPS, NONE','0 สำหรับค่าเริ่มต้น ส่วน 1 สำหรับหน้า uam').replace('SOCKS5','O').replace('proxy-type','opt').replace('ONLY SOCKS4, GET, HTTP, HTTPS, NONE','it 0 for default 1 for uam page').replace('BROWSER','COOKIE').replace('TRÌNHDUYỆT','bánhquy').replace('CHỈ SOCKS4, SOCKS5, HTTP, HTTPS, NONE','0 cho mặc định, 1 cho trang UAM').replace('loại proxy','opt')
